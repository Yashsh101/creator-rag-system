...
class IngestionService:
    ...
    def ingest_text_with_acl(
        ...
        """Persists plain text source chunks with ACL and chunk metadata."""
        try:
            normalized_text = text.strip()
            if not normalized_text:
                raise ValidationAppError("Text content is empty")

            content = normalized_text.encode("utf-8")
            content_hash = sha256_bytes(content + repr(sorted(metadata.items())).encode("utf-8"))
            existing = db.execute(
                select(Document).where(Document.content_hash == content_hash, Document.owner_id == owner_id)
            ).scalar_one_or_none()
            if existing:
            ...
            text_chunks = self.chunker.chunk_pages([ParsedPage(page_number=1, text=normalized_text)])
            if not text_chunks:
                raise ValidationAppError("No chunks were produced from the text content")

            embeddings = self.embedding_service.embed_texts([chunk.text for chunk in text_chunks])
            raw_file_uri = self.storage_backend.put_bytes(
                key=f"raw/{source_type}/{content_hash}/{filename}",
                content=content,
                content_type=content_type,
            )
            parsed_text_uri = self.storage_backend.put_bytes(
                key=f"parsed/{source_type}/{content_hash}/parsed.txt",
                content=content,
                content_type="text/plain; charset=utf-8",
            )
            ...
            for text_chunk, embedding in zip(text_chunks, embeddings, strict=True):
                db.add(
                    Chunk(
                        ...
                        page_start=text_chunk.page_start,
                        page_end=text_chunk.page_end,
                        section_path=source_type,
                        embedding=embedding,
                        metadata_json={**text_chunk.metadata, **metadata},
                    )
                )
