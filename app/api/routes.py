...
from app.schemas.query import ChatStreamRequest, CitationResponse, QueryRequest, QueryResponse
...
from app.services.langchain_service import get_chain
...
# LEGACY: not used in demo, kept for backwards compatibility
...
async def chat_stream(
    ...
    """Streams conversational RAG answers as server-sent events."""
    return StreamingResponse(_chat_stream_events(payload), media_type="text/event-stream")
...
async def _chat_stream_events(payload: ChatStreamRequest):
    try:
        chain = get_chain(payload.session_id)
        async for chunk in _chain_astream(chain, payload.query):
            if "answer" in chunk:
                yield _sse({"type": "token", "content": chunk["answer"]})
        ...
        logger.exception("chat_stream_failed", extra={"event": "chat_stream_failed", "session_id": payload.session_id})
        yield _sse({"type": "error", "message": str(exc)})


async def _chain_astream(chain, query: str):
