...
def run_worker(poll_interval_seconds: float = 2.0, stale_after_seconds: int = 900) -> None:
    worker_id = f"video-ingestion-worker-{uuid.uuid4().hex[:8]}"
    service = VideoIngestionJobService()
    logger.info("video_ingestion_worker_started", extra={"event": "video_ingestion_worker_started"})
    ...
    logger.info("video_ingestion_worker_stopped", extra={"event": "video_ingestion_worker_stopped"})
