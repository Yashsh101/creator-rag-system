...
class Settings(BaseSettings):
    ...
    # Durable conversation memory for the video chat graph.
    # "sqlite" survives restarts/multi-worker; "memory" = ephemeral fallback.
    chat_checkpointer_backend: str = "sqlite"  # sqlite | postgres | memory
    chat_checkpointer_path: str = "data/chat_checkpoints.sqlite"
    chat_checkpointer_pg_dsn: str = ""  # overrides DATABASE_URL for the PG checkpointer
    chat_checkpointer_pg_pool_size: int = 5
    # Video-path (Chroma) hybrid retrieval + per-video balancing.
    video_retrieval_top_k: int = 6
    video_hybrid_candidate_k: int = 20
    video_bm25_weight: float = 0.35
    video_rrf_k: int = 60
    # Vector store backend for the video path: chroma (default) | qdrant (scale).
    video_vectorstore_backend: str = "chroma"
    video_chroma_persist_dir: str = ""  # empty = in-memory; set a path to persist
    qdrant_url: str = ""
