...
from app.schemas.query import ChatStreamRequest, QueryRequest
...
def test_chat_stream_events_emit_sse_tokens(monkeypatch):
    class FakeChain:
        async def astream(self, payload):
            assert payload["question"] == "What worked?"
            yield {"answer": "Revenue "}
            yield {"answer": "expanded."}

    monkeypatch.setattr(routes, "get_chain", lambda session_id: FakeChain())

    async def collect():
        return [event async for event in routes._chat_stream_events(ChatStreamRequest(session_id="s1", query="What worked?"))]

    import asyncio

    events = asyncio.run(collect())

    assert 'data: {"type": "token", "content": "Revenue "}' in events[0]
    assert 'data: {"type": "token", "content": "expanded."}' in events[1]
    assert events[-1] == 'data: {"type": "done"}\n\n'


def test_chat_stream_route_is_registered():
    route_paths = {getattr(route, "path", "") for route in routes.router.routes}

    assert "/query/stream" in route_paths
